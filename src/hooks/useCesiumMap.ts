...
export function useCesiumMap(containerRef: React.RefObject<HTMLDivElement | null>) {
    ...
    (corridor: CorridorTrack) => {
      ...
      addEntity(`${corridor.id}-glow`, {
        polyline: {
          ...
          clampToGround: true,
          width: 20,
          material: Cesium.Color.fromCssColorString(T.green).withAlpha(0.1),
        },
      });
  ...
  // Draw corridor analysis overlays (nodes, inferred path, beliefs)
  const drawAnalysis = useCallback(
  ...
  // Main handler — matches MapParams interface
